...
let images2 =["img/Arctic_Fox.png", "img/Black_Bear.png", "img/Canada_Goose.png", "img/Canada_Lynx.png", "img/Greater_Short-Horned_Lizard.png", "img/Habour_Seal.png", "img/Mink.png", "img/Painted_Turtle.png",]
let images = ["imgs/001.jpg","imgs/002.jpg","imgs/003.jpg","imgs/004.jpg","imgs/005.jpg","imgs/006.jpg","imgs/007.jpg","imgs/008.jpg"]
...
// The following two functions are to be used to extract data between iframes.
function hasWon() {
  return win;
}
...
function checkMatch(card_type, card_id) {
  ...
  if (matchedIDs.includes(card_id))
    return;
/*
  ...
  matchedIDs.push(card_id);

  if (openedCards.length > 1) { // two cards flipped
    lock = 1;
    card1 = openedCards.pop(); // get first card
    card2 = openedCards.pop(); // get second card
    ...
    if (card1.isMatch(card2)) {
      ...
      var cards_to_flip = [];
      cards_to_flip.push(card1);
      cards_to_flip.push(card2);
      setTimeout(function() {
        cards_to_flip.forEach(function(item) {
          flipById(item.id);
        });
      }, 750);
    }
